...
public class PictureManager : MonoBehaviour
{
    ...
    public Vector2 StartPosition = new Vector2(-5f, 3.62f);
    public Vector2 StartPosition2 = new Vector2(-5f, 3.62f);
    public Vector2 StartPosition3 = new Vector2(-5f, 3.62f);
    ...
    public GameObject YoureScoreText;
    ...
    private Vector2 _offsetFor15Pairs = new Vector2(2.4f, 2.4f);
    private Vector2 _offsetFor20Pairs = new Vector2(2.3f, 2.3f);
    private Vector3 _newScaleDown = new Vector3(1.7f, 1.7f);
    ...
    void Start()
    {
        ...
        if(GameSettings.Instance.GetPairNumber() == GameSettings.EPairNumber.E10Pairs)
        {
            ...
            MovePicture(5, 2, StartPosition, _offset);
        }

        else if(GameSettings.Instance.GetPairNumber() == GameSettings.EPairNumber.E15Pairs)
        {
            CurrentGameState = GameState.MovingOnPositions;
            SpawnPictureMesh(4, 3, StartPosition2, _offset, false);
            MovePicture(4, 3, StartPosition2, _offsetFor15Pairs);
        }

        else if(GameSettings.Instance.GetPairNumber() == GameSettings.EPairNumber.E20Pairs)
        {
            CurrentGameState = GameState.MovingOnPositions;
            SpawnPictureMesh(6, 3, StartPosition3, _offset, false);
            MovePicture(6, 3, StartPosition3, _offsetFor20Pairs);
        }
    }
    ...
    private void LoadMaterials()
    {
        ...
        var textueFilePath = GameSettings.Instance.GetPuzzleCategoryTextureDirectoryName();
    ...
    public void ShowEndGameInformation()
    {
        ...
        if(config.IsBestScore())
        {
            ...
            YoureScoreText.SetActive(false);
        }
        else
        {
            NewBestScoreText.SetActive(false);
            YoureScoreText.SetActive(true);
        }
    ...
    public void MovePicture(int rows, int columns, Vector2 pos, Vector2 offset)
    {
        var index = 0;
        for(var col = 0; col < columns; col++)
        {
            for(int row = 0; row<rows; row++)
            {
                ...
                StartCoroutine(MoveToPosition(targetPostion, PictureList[index]));
                index++;
            }
        }
    }
